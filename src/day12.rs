...
fn calculate_paths_rec(
    paths: &HashMap<String, HashSet<String>>,
    mut already_visited_nodes: HashSet<String>,
    current_path: &[String],
) -> i64 {
    ...
    for destination in caves_we_can_go_to {
        ...
        paths_from_here += calculate_paths_rec(
            paths,
            already_visited_nodes.clone(),
            new_current_path.as_slice(),
        );
    }
...
fn part2(input: &[(String, String)]) -> i64 {
    ...
    calculate_paths_rec2(
        &paths,
        HashSet::new(),
        vec!["start".to_string()].as_slice(),
        0,
    )
}

fn calculate_paths_rec2(
    paths: &HashMap<String, HashSet<String>>,
    mut already_visited_nodes: HashSet<String>,
    current_path: &[String],
    number_of_single_cave_overrides: u8,
) -> i64 {
    ...
    for destination in caves_we_can_go_to {
        ...
        let mut new_current_path = current_path.to_owned();
        new_current_path.push(destination.clone());
        paths_from_here += calculate_paths_rec2(
            paths,
            already_visited_nodes.clone(),
            new_current_path.as_slice(),
            number_of_single_cave_overrides,
